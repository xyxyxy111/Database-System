...
class StorageEngine:
    ...
    def _save_metadata(self):
        ...
        # 清空页面数据区域并写入新的元数据
        page._data = bytearray(self.disk_manager.PAGE_SIZE - 24)  # 清空数据区
        if len(data) > 0 and len(data) < len(page._data):
            page.write_data(0, data)
    ...
    def create_table(
        ...
        # 创建表元数据
        metadata = TableMetadata(table_name, column_names, column_types)
        self.table_metadata[table_name] = metadata
    ...
    def insert_record(self, table_name: str, values: List[Any]) -> bool:
        ...
        # 在新页面中插入记录
        new_page = self.buffer_manager.pin_page(new_page_id)
        if new_page and self._try_insert_to_page(new_page, record_data):
            self.buffer_manager.unpin_page(new_page_id, True)
            metadata.record_count += 1
            self._save_metadata()
            return True

        if new_page:
            self.buffer_manager.unpin_page(new_page_id, False)
    ...
    def table_exists(self, table_name: str) -> bool:
        """检查表是否存在"""
        return table_name in self.table_metadata
