...
class BufferManager:
    ...
    def __init__(
        ...
        # 初始化替换策略相关数据结构
        self._init_replacement_policy()

        # 预读相关
        self.prefetch_window = 2  # 预读窗口大小
        self.last_access_page = -1  # 上次访问的页面
    ...
    def _init_replacement_policy(self):
        """初始化替换策略相关的数据结构"""
        if self.replacement_policy in [ReplacementPolicy.LRU, ReplacementPolicy.FIFO]:
            self.access_order = OrderedDict()
        elif self.replacement_policy == ReplacementPolicy.CLOCK:
            self.clock_hand = 0  # 时钟指针
            self.reference_bits: Dict[int, bool] = {}  # 页面引用位
            self.page_order: List[int] = []  # 页面在缓冲区中的顺序
        elif self.replacement_policy == ReplacementPolicy.LFU:
            self.frequency_counter: Dict[int, int] = {}  # 页面访问频率
    ...
    def get_page(self, page_id: int) -> Optional[Page]:
        ...
        if self.enable_prefetch:
            self._prefetch_pages(page_id)

        return page

    def _prefetch_pages(self, current_page_id: int):
        """预读机制 - 预测性加载后续页面"""
        # 顺序预读：假设页面访问具有局部性
        if self.last_access_page != -1:
            # 检测顺序访问模式
            if current_page_id == self.last_access_page + 1:
                # 顺序访问，预读后续页面
                for i in range(1, self.prefetch_window + 1):
                    prefetch_page_id = current_page_id + i
                    if (
                        prefetch_page_id not in self.buffer_pool
                        and len(self.buffer_pool) < self.buffer_size
                    ):
                        self._prefetch_single_page(prefetch_page_id)

        self.last_access_page = current_page_id

    def _prefetch_single_page(self, page_id: int):
        """预读单个页面"""
        try:
            page_data = self.disk_manager.read_page(page_id)
            if page_data:
                ...
                self.buffer_pool[page_id] = page
                self._update_access(page_id)
                self._log(f"PREFETCH: Page {page_id}")
        except Exception as e:
            self._log(f"PREFETCH FAILED: Page {page_id}, Error: {e}")
    ...
    def unpin_page(self, page_id: int, is_dirty: bool = False):
        ...
        if page_id in self.buffer_pool:
            ...
            self._log(
                f"UNPIN: Page {page_id} (pin_count={page.pin_count}, dirty={is_dirty})"
            )
    ...
    def _evict_page(self) -> bool:
        ...
        self._cleanup_victim_metadata(victim_page_id)
    ...
    def _cleanup_victim_metadata(self, page_id: int):
        """清理被驱逐页面的元数据"""
        if self.replacement_policy in [ReplacementPolicy.LRU, ReplacementPolicy.FIFO]:
            self.access_order.pop(page_id, None)
        elif self.replacement_policy == ReplacementPolicy.CLOCK:
            if page_id in self.page_order:
                self.page_order.remove(page_id)
            self.reference_bits.pop(page_id, None)
        elif self.replacement_policy == ReplacementPolicy.LFU:
            self.frequency_counter.pop(page_id, None)
    ...
    def _select_victim(self) -> Optional[int]:
        ...
        if self.replacement_policy == ReplacementPolicy.LRU:
            ...
            return self._select_victim_lfu()
        return None
    ...
    def _select_victim_fifo(self) -> Optional[int]:
        """FIFO选择被驱逐页面"""
        for page_id in self.access_order:
            if page_id not in self.pinned_pages:
                return page_id
    ...
    def _select_victim_clock(self) -> Optional[int]:
        ...
        while True:
            if self.clock_hand >= len(self.page_order):
                self.clock_hand = 0

            page_id = self.page_order[self.clock_hand]
            ...
            self.clock_hand = (self.clock_hand + 1) % len(self.page_order)
    ...
    def _select_victim_lfu(self) -> Optional[int]:
        """LFU选择被驱逐页面"""
        min_freq = float("inf")
        victim = None

        for page_id in self.buffer_pool:
            if page_id not in self.pinned_pages:
                freq = self.frequency_counter.get(page_id, 0)
                if freq < min_freq:
                    min_freq = freq
                    victim = page_id
    ...
    def _update_access(self, page_id: int):
        ...
        if self.replacement_policy == ReplacementPolicy.LRU:
            ...
            # Clock: 设置引用位
            self.reference_bits[page_id] = True
            if page_id not in self.page_order:
                self.page_order.append(page_id)
        elif self.replacement_policy == ReplacementPolicy.LFU:
            # LFU: 增加访问频率
            old_freq = self.frequency_counter.get(page_id, 0)
            self.frequency_counter[page_id] = old_freq + 1
...
def main():
    ...
    for i in range(5):
        ...
        # 写入测试数据
        test_data = f"Test data for page {page_id}".encode().ljust(
            dm.PAGE_SIZE, b"\x00"
        )
